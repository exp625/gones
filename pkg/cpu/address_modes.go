...
func (cpu *CPU) ABS() (uint16, uint8, uint8) {
...
// Abbreviation: a,x
// val = PEEK(arg + X)
func (cpu *CPU) ABX() (uint16, uint8, uint8) {
	...
	if location&0xFF00 != (high<<8) && cpu.Instructions[cpu.Bus.CPURead(cpu.PC)].ClockCycles == 4 {
...
// Abbreviation: a,y
// val = PEEK(arg + Y)
func (cpu *CPU) ABY() (uint16, uint8, uint8) {
	// Build location from high and low bits
	low := uint16(cpu.Bus.CPURead(cpu.PC + 1))
	high := uint16(cpu.Bus.CPURead(cpu.PC + 2))
	location := (high << 8) | low
...
func (cpu *CPU) ACC() (uint16, uint8, uint8) {
	return 0, cpu.A, 0
}
...
func (cpu *CPU) IDX() (uint16, uint8, uint8) {
...
func (cpu *CPU) IMM() (uint16, uint8, uint8) {
	location := cpu.PC + 1
	return location, cpu.Bus.CPURead(location), 0
}
...
func (cpu *CPU) IMP() (uint16, uint8, uint8) {
	return 0, 0, 0
}
...
func (cpu *CPU) IND() (uint16, uint8, uint8) {
...
func (cpu *CPU) IZY() (uint16, uint8, uint8) {
	// Build pointer from high and low bit
	offset := uint16(cpu.Bus.CPURead(cpu.PC + 1))
	...
	// high bits increased due to x offset
	if location&0xFF00 != (high<<8) && cpu.Instructions[cpu.Bus.CPURead(cpu.PC)].ClockCycles != 8 {
		return location, cpu.Bus.CPURead(location), 1
	}
	return location, cpu.Bus.CPURead(location), 0
}
...
func (cpu *CPU) REL() (uint16, uint8, uint8) {
...
func (cpu *CPU) ZP0() (uint16, uint8, uint8) {
...
// Abbreviation: d,x
// val = PEEK((arg + X) % 256)
func (cpu *CPU) ZPX() (uint16, uint8, uint8) {
	// Zero page is address range 0x0000 - 0x00FF
...
// Abbreviation: d,y
// val = PEEK((arg + Y) % 256)
func (cpu *CPU) ZPY() (uint16, uint8, uint8) {
	// Zero page is address range 0x0000 - 0x00FF
